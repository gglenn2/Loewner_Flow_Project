function [Circle] = PartitionCircle(radius, center, nPts)
...
% to trace out the desired points on the arc.
    ...
    for j=1:nArcPts 
        [X Y] = pol2cart((j-1)*delTheta, radius);
        Circle(j,1) = X;
        Circle(j,2) = Y;
    end 
...
% to obtain desired points on the line.
    ...
    for j=1:nLinePts+1
        Circle(j+nArcPts,:) = [-1*radius + j*delX 0];
    end
...
%     plot(Circle(1:nPts,1), Circle(1:nPts,2),'-r*');
