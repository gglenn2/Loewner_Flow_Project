...
clear Inputs warning titleStr prompt options dimWarning temp1 f list synErr;
...
for j=1:nTimeSteps-1   
    ...
    if (isList) 
        ...
        lam = double(L((j-1)*dt+t0));
    end
    
    % Test all points that have not yet been captured
    saveC = C;
    C(~C) = CapMatrix(Grid(~C), lam, dt);
